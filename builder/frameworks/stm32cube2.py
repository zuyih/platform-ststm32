...
STM32Cube packages handled by `stm32cube.py`:
    ...
    <family>xx_drivers/templates/common
                                  default <family>xx_hal_conf.h, stm32_assert.h
...
cpppath = [
    ...
    # `stm32_assert.h` is only shipped in this folder, so it stays on the
    # include path even when the project provides its own hal_conf header.
    # Keep it last so that a project can still override the shipped defaults.
    TEMPLATES_DIR,
]
